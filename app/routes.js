...
var additionalFunctions = require('../test')
var T = new Twit({
    ...
    app.get('/test', (req, res) => {
        ...
        request(options)
            .then(fbRes => {
                fs.writeFile(file, JSON.stringify(JSON.parse(fbRes), null, 4), (err) => {if (err) console.log(err)})
                FB.api(url, {access_token: user.facebook.token}, function(response){
                    handleResponse(response, file, user, res, url);
                });
            })
    })

    app.get('/compare', (req, res) => {
        additionalFunctions.read(function(commonFriends){
